...
public class Inventories {
	...
	public static Inventory MainGui(Player p) {
		Inventory inv = Bukkit.createInventory(p, 9 * 3, "§8Stats: " + p.getName());
		inv.setItem(26, new ItemBuilder(Material.ARROW).setName("§cClose").toItemStack());

		for (StatisticCategory category : StatisticCategory.values()) {
			ItemBuilder builder = new ItemBuilder(category.getGuiMaterial()).setName("§7" + category.getName()).addLoreLine("").addLoreLine("§7Events Listed");
	...
	public static Inventory CategoryGui(Player p, StatisticCategory category) {
		Inventory inv = Bukkit.createInventory(p, 3 * 9, "§8Stats: " + p.getName());
		inv.setItem(26, new ItemBuilder(Material.ARROW).setName("§cBack").toItemStack());
		...
		if(category == StatisticCategory.LIVES) {
			...
			DateFormat timeFormated = DateFormat.getTimeInstance();
